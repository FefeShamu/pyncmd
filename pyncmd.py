...
parser.add_argument('--phone', metavar='PHONE',
                    help='Phone number to your account')
parser.add_argument('--password', metavar='PASSWORD',
                    help='Password to your account')
parser.add_argument('--port', metavar='PORT',
                    help='Port to be listened on', default='3301')
parser.add_argument('--message', metavar='MSG',
                    help='Custom message to be displayed', default='')

args = parser.parse_args()
args = args.__dict__

port, phone, password, ContributerMessage = int(
    args['port']), args['phone'], args['password'], args['message']
# Parsing argumnets
...
if os.path.exists('.cookies'):
    ...
    try:
        pyncm.ncm.session.cookies.update(json.loads(
...
if phone and password:
    ...
    open('.cookies', 'w+',
         encoding='utf-8').write(json.dumps(pyncm.ncm.session.cookies.get_dict()))
...
def API(request: RequestHandler):
    ...
    try:
                ...
                try:
                    ...
                    if 'code' in response[requirement].keys() and response[requirement]['code'] != 200:
                        response[requirement][
                            'message'] = f"netease eAPI error:{response[requirement]['code']}"
                    else:
        ...
        response = {**response,
                    'requirements': requirements, 'required_id': id}
        ...
        request.end_headers()
        HTTPModules.WriteString(
            request, '{"message":"unexcepted error:%s"}' % e)
    count += 1


@server.route(PathMakerModules.DirectoryPath('/static/'))
def html(request: RequestHandler):
    # Adds '.',referncing local paths
    HTTPModules.WriteStream(request, './html' + request.path)
# Don't index folders
